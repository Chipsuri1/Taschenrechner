    ...
    public void visit(BinOpNode node) {
        ...
        if (node.getRight() instanceof SyntaxNode && node.getLeft() instanceof SyntaxNode) {
            ...
            switch (node.getOperator()) {
                case "+" -> node.setValue(leftIntegerNode.getValue() + rightIntegerNode.getValue());
                case "-" -> node.setValue(leftIntegerNode.getValue() - rightIntegerNode.getValue());
                case "*" -> node.setValue(leftIntegerNode.getValue() * rightIntegerNode.getValue());
                case "/" -> node.setValue(leftIntegerNode.getValue() / rightIntegerNode.getValue());
                case "^" -> node.setValue((int) Math.pow(leftIntegerNode.getValue(), rightIntegerNode.getValue()));
                default -> throw new RuntimeException(("BinOpNode - forbidden Operator: " + node.operator));
            }
        } else {
